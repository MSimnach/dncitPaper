...
#' @param dncits vector of all DNCITs, e.g. c("RCOT", "CMIknn", "CODEC", "FCIT", "kpc_graph_2_10", "PCIT", "FCIT")
...
create_test_plot <- function(folder_path, dncits, settings, sample_sizes=NULL, betas=NULL, runtimes_plot=FALSE){
  ...
  for(dncit in dncits){
    files_with_CMIknn <- list.files(path = folder_path, pattern = dncit, full.names = TRUE)
  ...
  # Combine plots
  p1 <- cowplot::plot_grid(plotlist=c(list(y_axis),results_plots),
...
create_empty_result_tab <- function(settings_files,sample_sizes=NULL, betas=NULL){
  if(is.null(betas)){
    ...
    result_tab <- data.frame(matrix(nrow=length(settings_files), ncol = length(betas)+1))
...
plot_dncit_col <- function(dncit, result_tab_dncit, legend=FALSE){
...
cit_name <- function(dncit){
  if(dncit == 'RCOT'){
    return('Deep-RCoT')
  }else if(dncit == 'CMIknn'){
    return('Deep-CMIknn')
  }else if(dncit == 'CODEC'){
    return('Deep-CODEC')
  }else if(dncit == 'FCIT'){
    return('Deep-FCIT')
  }else if(dncit == 'kpc_graph_2_10'){
    return('Deep-CPT-KPC')
  }else if(dncit == 'PCIT'){
    return('Deep-PCIT')
  }else if(dncit == 'GCM'){
